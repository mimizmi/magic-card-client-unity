...
| The login screen shows connection state, and a dropped link does not erase a refusal | `LoginViewModelTests` (10 tests), `LoginViewTests` (PlayMode, 1 test) | **Mutation-verified.** Forcing `CanSubmit` to `true` fails two tests. `ResultText` and `ConnectionFaultText` are separate fields so that a fault arriving mid-read cannot overwrite the message the user is looking at. |
